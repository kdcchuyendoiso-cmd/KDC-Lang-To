...
# --- CSS TÙY CHỈNH GIAO DIỆN (CHIA 2 CỘT DI ĐỘNG & THU NHỎ GIAO DIỆN) ---
st.markdown("""
<style>
    /* Tối ưu hóa kích thước cột bên (Sidebar) vừa vặn cho điện thoại & máy tính */
    [data-testid="stSidebar"] {
        min-width: 230px;
        max-width: 270px;
    }
    
    /* Thu nhỏ kích thước chữ và khoảng cách trong menu Sidebar */
    ...
    /* Thu nhỏ chữ và tối ưu hóa hiển thị bảng dữ liệu */
    [data-testid="stDataFrame"] div, [data-testid="stDataEditor"] div {
        font-size: 12.5px !important;
    }
    
    /* Tùy chỉnh khung hiển thị nhanh bên phải trên màn hình di động */
    .quick-view-card {
...
# --- THANH BÊN (SIDEBAR) CHỨA 10 TAB CHỨC NĂNG ---
with st.sidebar:
    st.markdown("""
        <div class="sidebar-header-box">
            <h2>🏘️ KDC Lăng Tô</h2>
            <p>Hệ thống số hóa cộng đồng</p>
        </div>
    """, unsafe_allow_html=True)
...
st.sidebar.markdown("""
    <div class="sidebar-footer-note">
        Chương trình mục tiêu quốc gia xây dựng nông thôn mới và phát triển KT-XH vùng đồng bào DTTS giai đoạn 2026 - 2035.
    </div>
""", unsafe_allow_html=True)
st.sidebar.markdown("---")

# --- BỐ CỤC CHIA 2 PHẦN TRÊN MÀN HÌNH CHÍNH (DÙNG ĐỂ HIỂN THỊ KÈM THÔNG BÁO NHANH) ---
# Nếu người dùng đang ở tab Thông Báo hoặc Sự Kiện, ta có thể kết hợp hiển thị song song tổng quan.
# Đối với toàn bộ ứng dụng, chúng ta thiết kế giao diện chia cột responsive (cột trái nội dung chính, cột phải hiển thị nhanh tin tức/sự kiện tiêu biểu).

col_main, col_quick = st.columns([2.5, 1], gap="medium")

with col_quick:
    st.markdown("### 📌 Bảng Tin Nhanh")
    # Lấy nhanh 3 thông báo mới nhất hoặc được ghim
    df_tb_quick = load_excel_data("ThongBao")
    if not df_tb_quick.empty:
        pinned = df_tb_quick[df_tb_quick['Ghim Nổi Bật'] == 'Có']
        display_quick = pinned if not pinned.empty else df_tb_quick.head(3)
        for _, r in display_quick.iterrows():
            st.markdown(f"""
                <div class="quick-view-card">
                    <b>📢 {r.get('Tiêu Đề', '')}</b><br>
                    <small>📅 {r.get('Ngày Đăng', '')} | 🏷️ {r.get('Phân Loại', '')}</small><br>
                    <p style="margin: 5px 0 0 0; font-size: 12px;">{r.get('Nội Dung', '')[:90]}...</p>
                </div>
            """, unsafe_allow_html=True)
    else:
        st.info("Chưa có thông báo nhanh.")

    st.markdown("### 🎉 Sự Kiện Sắp Tới")
    df_sk_quick = get_updated_events_df()
    if not df_sk_quick.empty:
        for _, r in df_sk_quick.head(2).iterrows():
            st.markdown(f"""
                <div class="quick-view-card" style="border-left-color: #28a745;">
                    <b>🎉 {r.get('Tên Sự Kiện', '')}</b><br>
                    <small>⏰ {r.get('Thời Gian Bắt Đầu', '')} | 📍 {r.get('Địa Điểm', '')}</small>
                </div>
            """, unsafe_allow_html=True)
    else:
        st.info("Chưa có sự kiện nổi bật.")

with col_main:
    # --- XỬ LÝ GIAO DIỆN CÁC CHỨC NĂNG CHÍNH ---

    if "1. 📢 Bảng Tin & Thông Báo" in choice:
        st.header("📢 Bảng Tin & Thông Báo Chi Tiết")
        ...
        st.header("🎉 Sự Kiện Cộng Đồng Chi Tiết")
        ...
        if not st.session_state.authenticated:
            ...
            st.success("✅ Cán bộ đã đăng nhập thành công. Bạn có thể tự do chỉnh sửa dữ liệu trực tiếp trên các bảng bên dưới.")
            ...
            with tab_cq:
                st.subheader("🛒 Quản lý Chợ Quê Nông Sản")
